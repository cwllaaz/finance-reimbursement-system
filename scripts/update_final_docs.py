...
modules = [
    ("首页仪表盘", "财务/领导可见，员工不可见；部门负责人只看本部门；六张统计卡片可跳转到真实筛选结果。"),
    ("统一后台界面", "固定深色侧边栏、统一顶部栏、独立页面路由和响应式布局；支持菜单折叠、刷新保持页面及浏览器前进后退。"),
...
def project_report():
    ...
    add_table(doc, ["类别", "完成内容"], [
        ...
        ("界面能力", "独立路由、统一后台布局、响应式页面、统一列表和表单交互、刷新保持页面及前进后退。"),
    ...
    add_bullets(doc, [
        ...
        "已完成桌面、笔记本和手机宽度的页面检查，菜单、表格、弹窗和图表可正常使用。",
