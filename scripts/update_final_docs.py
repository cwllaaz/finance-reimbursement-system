...
roles = [
    ...
    ("COMMITTEE", "院务委员会", "查看全院业务单据和完整资产台账。"),
    ("CASHIER", "出纳", "处理付款任务，登记付款日期、金额和凭证号。"),
    ("ADMIN", "管理员", "用户管理、系统维护、全量数据查看和安全审计。"),
]

modules = [
    ("首页仪表盘", "财务/领导可见，部门负责人只看本部门；六张统计卡片、双图表、最近业务单据和待办事项组成三层布局。"),
    ("报销状态图", "外部引导线显示状态名称、数量和占比，中心显示报销单总数，图例固定在底部并自动避让标签。"),
    ("首页待办事项", "调用统一工作台接口，显示真实总数和最近 5 条待办，包含业务、编号、标题、申请人、金额、节点和等待时间。"),
    ("统一后台界面", "固定深色侧边栏、统一顶部栏、独立路由和响应式布局；菜单按六类业务分组，空分组按角色权限隐藏。"),
    ("我的申请", "由后端统一工作台返回本人创建的报销、申购、劳务和借款。"),
    ("我的待办", "按角色、审批节点和部门范围返回真实待办，可直接完成审批、驳回、付款、财务复核和冲账。"),
    ("已办事项", "根据审批记录返回当前用户实际审批、付款或复核过的事项。"),
    ("报销管理", "审批编号 BX+日期+序号；提交时四项关键资料和发票必填/必传，其他凭证按需上传；OCR 金额校验。"),
    ("申购管理", "编号 CG+日期+序号；多明细；总额大于 5 万元时院务委员会材料必传。"),
    ("资产出入库/验收", "编号 LY/ZC；自动记录办公室验收人，由办公室选择实际使用人办理领用。"),
    ...
    ("完整单据 PDF", "报销、申购、劳务和借款支持下载打印归档 PDF，包含基本信息、明细、审批、付款复核及附件清单。"),
...
accounts = [
    ...
    ("committee", "123456", "院务委员会", "查看全院单据和完整资产台账"),
...
def feature_summary():
    ...
    add_bullets(doc, [
        ...
        "报销单可先保存草稿；提交审批时申请人电话、银行账号、收款人全称、报销事由必须填写，并至少上传一份发票，其他凭证按实际业务上传。",
        "报销金额超过 5 万元时必须填写大额说明并上传会议材料；申购总额大于 5 万元时必须上传院务委员会审议材料。",
        "资产验收入库自动记录办公室验收人；办理领用时必须选择实际使用人，且不能与验收人相同。",
        "首页报销总数、本月金额、已通过、待审批、待冲账和逾期卡片可直接进入对应筛选页面。",
    ...
    add_bullets(doc, [
        "前端已拆分为 layouts、views、components、api、composables 和 router，菜单已整合为仪表盘、工作台、业务模块、台账和系统管理。",
        "左侧菜单按工作台、申请业务、资产管理、财务处理、预算统计和系统管理分组；我的待办显示实时数量徽标，出纳付款统一在待办中处理。",
        "首页采用六卡片、双图表、最近业务单据和待办事项三层布局，环形图显示引导线、数量、占比及中心总数。",
        "8 类演示角色、18 条前端路由权限校验通过；无权限页面会跳转到当前角色的默认页面。",
        "已检查桌面、笔记本和手机宽度，侧边栏、顶部栏、表格及弹窗没有页面级横向溢出。",
        "后端 75 项测试全部通过并完成打包，覆盖业务流程、角色权限、安全机制、PDF 和筛选边界。",
        "演示账号覆盖员工、部门负责人、财务、办公室、执行院长、院务委员会、出纳、管理员。",
...
def user_manual():
    ...
    add_numbered(doc, [
        "使用 employee 登录，新建报销草稿；补齐申请人电话、银行账号、收款人全称和报销事由，上传至少一份发票后提交，其他材料按实际业务上传。",
        "打开报销详情，执行 OCR 识别，检查金额校验提示，确认 OCR 数据。",
        "提交报销单，切换 finance、manager、executive、cashier 按流程审批和付款。",
        "finance 完成财务复核后，查看预算扣减和审批时间轴。",
        "使用 office 登录办理资产验收入库，确认系统自动显示验收人；再选择实际使用人办理领用。",
        "使用 admin 登录查看操作日志，确认提交、验收和领用等关键操作已经被记录。",
    ...
    doc.add_heading("五、页面导航与登录状态", level=1)
    add_bullets(doc, [
        "每个菜单都拥有独立地址，刷新页面后仍停留在当前页面，浏览器前进和后退按钮可以正常使用。",
        "无权访问某个页面时，系统会自动跳转到该角色的默认页面，不能依靠手动修改地址绕过权限。",
        "登录过期时系统会清除旧登录状态并返回登录页，重新登录后即可继续使用。",
        "在手机宽度下侧边栏自动收窄，表格可以横向滚动，内容较多的弹窗在内部滚动。",
    ])
    ...
    add_bullets(doc, [
        ...
        "部门负责人只能查看本部门数据。",
        "报销提交时必须具备四项关键资料和至少一份发票；其他凭证非必传，草稿阶段可以暂不完整。",
        "申购金额等于 5 万元时无需审议材料，大于 5 万元时必须上传院务委员会审议材料。",
        "资产领用由办公室办理，必须选择实际使用人，且不能选择本次验收人。",
...
def project_report():
    ...
    add_table(doc, ["类别", "完成内容"], [
        ...
        ("展示能力", "首页可点击统计卡片、工作台、时间轴、图表、Excel 导出、OCR 校验。"),
        ("归档能力", "报销、申购、劳务和借款可下载完整单据 PDF，包含审批记录、付款复核信息和附件清单。"),
        ("界面能力", "分组菜单、实时徽标、三层仪表盘、独立路由、响应式页面、统一列表和表单交互。"),
        ("凭证规则", "报销提交必须补齐关键资料并具有发票，其他凭证按需上传；大额申购必须具有院务委员会审议材料。"),
        ("资产管理", "分别记录验收人、领用人、当前保管人、验收时间、领用时间和流转历史。"),
        ("权限能力", "8 类角色，菜单权限和后端 API 权限同步控制。"),
    ...
    add_bullets(doc, [
        "后端 75 项测试全部通过并完成打包验证。",
        "前端生产构建通过，8 类角色和 18 条路由权限自动校验通过。",
        "已完成桌面、笔记本和手机宽度检查，六卡片布局、菜单折叠、图表恢复、待办列表和历史导航均正常。",
