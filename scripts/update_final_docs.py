...
modules = [
    ...
    ("报销管理", "审批编号 BX+日期+序号；提交时发票和其他凭证必传；OCR 金额校验。"),
...
def feature_summary():
    ...
    add_bullets(doc, [
        ...
        "报销单可先保存草稿；提交审批时必须至少上传一份发票和一份合同、会议记录、付款凭证或其他证明材料。",
    ...
    add_bullets(doc, [
        ...
        "左侧菜单按工作台、申请业务、资产管理、财务处理、预算统计和系统管理分组；我的待办与付款任务显示实时数量徽标。",
        "首页采用六卡片、双图表、最近业务单据和待办事项三层布局，环形图显示引导线、数量、占比及中心总数。",
        "7 类演示角色、18 条前端路由权限校验通过；无权限页面会跳转到当前角色的默认页面。",
        "已检查桌面、笔记本和手机宽度，侧边栏、顶部栏、表格及弹窗没有页面级横向溢出。",
        "后端 60 项测试全部通过并完成打包，覆盖业务流程、角色权限、安全机制和筛选边界。",
...
def user_manual():
    ...
    add_numbered(doc, [
        "使用 employee 登录，新建报销草稿；进入详情分别上传至少一份发票和一份其他凭证后再提交。",
...
def project_report():
    ...
    add_table(doc, ["类别", "完成内容"], [
        ...
        ("凭证规则", "报销提交必须具有发票和其他凭证；大额申购必须具有院务委员会审议材料。"),
    ...
    add_bullets(doc, [
        "后端 60 项测试全部通过并完成打包验证。",
        "前端生产构建通过，7 类角色和 18 条路由权限自动校验通过。",
