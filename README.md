...
- 首页仪表盘：部门负责人查看本部门，财务、执行院长和管理员查看全院；六张统计卡片可跳转到对应筛选结果。
...
- 报销管理：`BX + 日期 + 三位序号`，提交时必须同时具有发票和其他凭证，支持关联申购、百度/模拟 OCR 和金额校验。
...
报销单保存草稿时可暂不上传附件，提交审批时必须至少上传一份发票和一份其他凭证。金额超过 5 万元的报销还需填写大额说明并上传会议审议材料；申购总额大于 5 万元时必须上传院务委员会审议材料。
